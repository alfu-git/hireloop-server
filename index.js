...
async function run() {
  try {
    ...
    // post new job
    app.post("/jobs", verifyToken, verifyAdmin, async (req, res) => {
      const job = req.body;
      const newJob = {
        ...job,
        createdAt: new Date(),
    ...
    // post company
    app.post("/companies", verifyToken, verifyRecruiter, async (req, res) => {
    ...
    app.get("/my-companies", verifyToken, verifyRecruiter, async (req, res) => {
      const query = {};
